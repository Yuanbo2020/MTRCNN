...
def forward_total(model, generate_func, cuda, return_names = False):
    output_total = []
    label_total = []
    ...
    for num, data in enumerate(generate_func):
        (batch_x,  batch_y_arousal, batch_y_vanlence, batch_y_total) = data
...
def evaluate_total(model, generate_func, cuda):
    # Forward
    dict = forward_total(model=model, generate_func=generate_func, cuda=cuda)
...
def training_process_total(generator, model, models_dir, epochs, batch_size, lr_init=1e-3, cuda=1):
    ...
    for iteration, all_data in enumerate(generator.generate_training()):
        (batch_x, batch_y_arousal, batch_y_vanlence, batch_y_total) = all_data

        batch_x = move_data_to_gpu(batch_x, cuda)
        batch_y_total = move_data_to_gpu(batch_y_total, cuda)
        ...
        if iteration % check_iter == 0 and iteration > 1:
            ...
            val_acc_total = evaluate_total(model=model, generate_func=generate_func, cuda=cuda)

            print('E: ', '%.3f' % (Epoch), 'val_acc_total: %.3f' % float(val_acc_total) )
