...
def main(argv):
    ...
    models = ['CNN_Transformer', 'YAMNet', 'MobileNetV2', 'PANNs', 'PANNs_PreW', 'MTRCNN']
    model_index = models.index(model_type)
    using_models = [CNN_Transformer, YAMNet, MobileNetV2, PANN, PANN, MTRCNN]

    Dataset_dir = os.path.join(os.getcwd(), 'Dataset')

    model_name = 'Aro_val_' + model_type + '.pth'

    model = using_models[model_index](5)
    ...
    dict = forward_total(model=model, generate_func=generate_func, cuda=1)

    # softmax classification acc
    average_acc = cal_softmax_classification_accuracy(dict['label_total'], dict['output_total'], average='macro')
